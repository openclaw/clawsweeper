...
async function exercise(mode: "boundaries" | "deadline") {
  ...
  try {
    ...
    const source = `
      ...
      const finish=()=>{
        send({method:'item/completed',params:{threadId:'thread',turnId:'turn',item:{type:'agentMessage',text:JSON.stringify({responses,handoffRemoved:!ownHandoffs})}}});
