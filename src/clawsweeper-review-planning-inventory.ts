    ...
    for (let offset = 0; offset < numbers.length; offset += PR_ACTIVITY_REVISION_QUERY_PAGE_SIZE) {
      ...
      try {
        const response = ghJson<unknown>([
          ...
          "graphql",
          "-f",
          `query=${plannedPrActivityRevisionQuery(owner, name, page)}`,
        ]);
        ...
        for (const number of page) {
          revisions[String(number)] = activityRevision(repository[`pr_${number}`]);
        }
      } catch (error) {
        console.error(
...
function plannedPrActivityRevisionQuery(owner: string, name: string, numbers: readonly number[]) {
...
function activityRevision(value: unknown): string | null {
