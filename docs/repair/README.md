...
Each run persists a bounded schema-v2 `validation_proof` trace in the fix execution report and merge preflight: exact validated head/base SHAs, risk signals, command ID/digest/kind/source, stage, status, duration, reason, prerequisite, and digest-only subsumption provenance. Command IDs must carry the command-digest prefix, and the plan ID is recomputed from the trace topology before authorization. It contains no raw logs, command arguments, credentials, absolute paths, or hosts. Operator logs and prompts receive only compact stage/count summaries. Before push, the executor reruns staged proof whenever the final checkpoint or history compaction changed the validated head. Repair merge post-flight requires a passed trace whose head matches both the fix action and live PR head and whose base matches the live PR base; stale or internally inconsistent proof blocks merge. Earlier failed attempts remain visible after a successful repair retry.
