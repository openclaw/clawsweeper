...
The normal review publisher also dispatches this same worker for newly reviewed
issues and bounded backfill from existing open issue reports when automatic
issue implementation is enabled. Codex discovers the implementation and
validation from the repository; deterministic intake still blocks
protected/security signals, opt-outs, stale issue state, queued issue jobs, and
duplicate or linked PRs.
