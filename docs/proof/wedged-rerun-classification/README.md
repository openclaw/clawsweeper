...
- Tested source head: `e00105a696c6e7c8144fc754035447bd69499067`
...
[`red-green-transcript.txt`](red-green-transcript.txt) records the local fixture result. [`container-transcript.txt`](container-transcript.txt) is the exact-source Docker-backed Crabbox run: the merge-base implementation fails both new behavior fixtures, while the source head passes the five focused API/render/CLI/public-containment scenarios, `check:dashboard-strict`, and the complete `pnpm run check` gate (3,533 tests, 3,525 passed, zero failed, eight skipped). [`container-provenance.json`](container-provenance.json) freezes the provider, lease, run, image, hashes, timing, and bounded discarded attempts. [`container-secret-scan.json`](container-secret-scan.json) records a clean proof-directory scan.
...
Limits: GitHub API behavior is exercised through loopback fixtures rather than mutating production run 31910632853. The receipt-containing evidence commit changes proof files only after tested source head `e00105a696c6e7c8144fc754035447bd69499067`; no runtime source follows the receipt. The source and evidence commits are pushed together.
