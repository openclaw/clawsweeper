#!/usr/bin/env bash
# Crabbox local-container proof for the record front matter body-scope fix.
#
...
# (dist/clawsweeper-record-metadata.js) that every apply and review lane reads its
# record fields through.
#
...
if git rev-parse HEAD >/dev/null 2>&1 && [ -n "$BASE" ] \
  ...
  echo "FAIL: no pre-fix module. Run docs/proof/record-front-matter-body-scope/stage-before.sh first."
  exit 1
fi
