...
against a strict, stateful synthetic GitHub transport. Unknown commands fail;
there is no real-gh fallback and no inherited credential. Reports, runtime
artifacts, items, closed records, plans and canonical baselines are private
fixtures. The inherited private `TMPDIR` is preserved. The synthetic target
...
Both main and the candidate execute the same scenarios. The adapter triggers
...
This is a general same-author pair proof using supported legacy reports without
...
changes. Verified with this repair on Node 24.20.0.
