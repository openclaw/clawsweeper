...
The baseline requires migration proof for both the full and production-normalized Reporting patch. The candidate removes those false holds. Binary reads through bare, member, optional, awaited, parenthesized, bracket-member, and wrapped state paths remain blocked without compatibility proof, as do real JSON writes, browser storage, incomplete known persistence owners, and SQLite DDL. A state-path hint and file-read evidence cannot combine across separate hunks. Within one semantic hunk, the existing policy stays conservative: unrelated state-path and source-read signals can still require compatibility review. The recipe records that limitation; it does not treat patch evidence as JavaScript dataflow analysis.
