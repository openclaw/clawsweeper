// Compiled before/after proof for Codex process-tree cleanup on repeated worker signals
// and on a direct child that exits before the SIGKILL escalation fires.
//
// Both arms start the real compiled Codex process worker with a fake `codex` binary that
// spawns a SIGTERM-ignoring grandchild and never exits on its own, then send SIGTERM to
// the worker itself, the signal a job cancellation or an outer deadline delivers, and
// record whether the child and grandchild are still alive three seconds later. Three
// scenarios run per arm: the direct child ignores a single SIGTERM (the control: both arms
// stop the tree through the one-second SIGKILL escalation), the direct child exits on
// SIGTERM while its grandchild ignores it, and repeated SIGTERM while both ignore it. The
// baseline arm compiles src/codex-process-worker.ts from the base commit (default: merge
// base with origin/main, or HEAD~1 once the change is on main) in an isolated copy of
// src/; the candidate arm uses the current dist/.
//
// POSIX only: the worker's process-group handling has no Windows equivalent here.
//
// Usage: node docs/proof/codex-worker-signal-termination/run-proof.mjs [--base <rev>]
...
const WORKER = "src/codex-process-worker.ts";
...
const outDir = resolve(repoRoot, option("--out", ".artifacts/codex-worker-signal-termination"));
...
const fakeCodex = (childIgnoresSigterm) => `#!/usr/bin/env node
...
async function runScenario(arm, scenario) {
  const root = mkdtempSync(join(tmpdir(), "clawsweeper-codex-signal-proof-"));
  ...
  const worker = spawn(process.execPath, [join(arm.dist, "codex-process-worker.js"), optionsPath], {
    cwd: root,
  ...
  try {
    ...
    while (!existsSync(pidPath)) {
      if (Date.now() > startDeadline) throw new Error("fake Codex child never started");
      await sleep(50);
    }
...
if (!existsSync(join(candidateDist, "codex-process-worker.js"))) {
  throw new Error("dist/codex-process-worker.js is missing; run pnpm run build first");
}
...
// Before the change the worker exits cleanly once the direct child closes but never
// escalates to the rest of the process group, so the grandchild outlives it.
