...
SOURCE="src/clawsweeper-label-mutations.ts"
BASE="${PROOF_BASE:-$(git merge-base HEAD main 2>/dev/null || git merge-base HEAD origin/main)}"

mkdir -p "$PROOF_DIR/before"
git show "$BASE:$SOURCE" >"$PROOF_DIR/before/label-mutations-before.ts"

echo "staged $PROOF_DIR/before/label-mutations-before.ts"
echo "  base:   $BASE"
echo "  source: $SOURCE"
echo "  sha256: $(shasum -a 256 "$PROOF_DIR/before/label-mutations-before.ts" | cut -d' ' -f1)"
