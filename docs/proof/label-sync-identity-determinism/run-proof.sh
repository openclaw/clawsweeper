#!/usr/bin/env bash
# Crabbox local-container proof for the label sync identity determinism fix.
#
# Runs inside a Node 24 Linux container on the synced current checkout. It builds
# the main lane and exercises the shipped label mutation factory
# (dist/clawsweeper-label-mutations.js) that stamps every published issue label edit
# with the identity the action ledger dedupes on.
#
# See run-proof.mjs for the fixtures and assertions.
set -euo pipefail

PROOF_DIR="docs/proof/label-sync-identity-determinism"
...
test -f dist/clawsweeper-label-mutations.js \
  || { echo "FAIL: build did not produce dist/clawsweeper-label-mutations.js"; exit 1; }
echo "shipped comparator: $(node -p "/localeCompare/.test(require('fs').readFileSync('dist/clawsweeper-label-mutations.js','utf8')) ? 'localeCompare (pre-fix)' : 'code unit'")"
echo
...
STAGED="$PROOF_DIR/before/label-mutations-before.ts"
...
if git rev-parse HEAD >/dev/null 2>&1 && [ -n "$BASE" ] \
   && git cat-file -e "$BASE:src/clawsweeper-label-mutations.ts" 2>/dev/null; then
  echo "base: $BASE (re-derived from git)"
  mkdir -p "$PROOF_DIR/before"
  git show "$BASE:src/clawsweeper-label-mutations.ts" >"$STAGED"
elif [ -f "$STAGED" ]; then
  ...
  echo "FAIL: no pre-fix module. Run docs/proof/label-sync-identity-determinism/stage-before.sh first."
  exit 1
fi
cp "$STAGED" dist/label-mutations-before.ts
pnpm exec tsc dist/label-mutations-before.ts \
    --ignoreConfig --module nodenext --target es2023 --skipLibCheck \
  >"$ARTIFACT_DIR/before-build.log" 2>&1 || true
if [ -f dist/label-mutations-before.js ]; then
  set +e
  node "$PROOF_DIR/run-proof.mjs" --module dist/label-mutations-before.js \
  ...
  # A batch must already publish a mutation before the fix; that is what shows the
  # change only affects ordering, not whether the sync happens at all.
  if grep -q "^  FAIL  a batch publishes an issue_labels_sync mutation" \
      "$ARTIFACT_DIR/before-output.txt"; then
    echo "FAIL: the pre-fix module did not publish a label sync at all"
    exit 1
  fi
