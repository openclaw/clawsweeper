...
**Status (verified 2026-07-22 at 04:55 UTC):** PR 1 through PR 4, the rollout
...
[`openclaw/openclaw#108676`](https://github.com/openclaw/openclaw/pull/108676), so their
...
cause. Duplicate-item admission serialization and the size-4 rollback landed as
[`openclaw/clawsweeper#772`](https://github.com/openclaw/clawsweeper/pull/772)
at `883ce9914e57733b18b96bcf70e9308a29c8e237`. Its CI, CodeQL, and dashboard
deployment passed, and the public surface returned to `max_items=4` with a
60-second wait. Recovery run
[29891989901](https://github.com/openclaw/clawsweeper/actions/runs/29891989901)
then selected four distinct durable items, published state commit
[`7993abb2`](https://github.com/openclaw/clawsweeper-state/commit/7993abb2b64e71b81b9f30904fdb23182fb82558),
and reported `materialized=4`, `accepted=4`, `retryable=0`, and `released=0`.
The independent writer-identity signal did not recur. Pre-rollback run
[29891630162](https://github.com/openclaw/clawsweeper/actions/runs/29891630162)
also proved deferred collision members remained consumable: it selected one
event each for [`openclaw/openclaw#106482`](https://github.com/openclaw/openclaw/pull/106482)
and [`openclaw/openclaw#108676`](https://github.com/openclaw/openclaw/pull/108676) among eight distinct items and completed
state commit
[`4b65e1a6`](https://github.com/openclaw/clawsweeper-state/commit/4b65e1a628bba833d56d4b23247e7d215994a065)
with `materialized=8`, `accepted=8`, and `retryable=0`. The rollout is therefore
authorized to re-deliver size 8, which must still pass both safety and
...
| Size-8 capacity step                          | Safety gate failed; rolled back                | [`openclaw/clawsweeper#771`](https://github.com/openclaw/clawsweeper/pull/771) landed at `5808a1829c8e7c584ce292b8a4649a5cd5bfe17b`; CI [run 29889947460](https://github.com/openclaw/clawsweeper/actions/runs/29889947460), CodeQL [run 29889947502](https://github.com/openclaw/clawsweeper/actions/runs/29889947502), and dashboard deployment [run 29890185892](https://github.com/openclaw/clawsweeper/actions/runs/29890185892) passed. Production run [29890385881](https://github.com/openclaw/clawsweeper/actions/runs/29890385881) failed before commit because two queue events for [`openclaw/openclaw#108676`](https://github.com/openclaw/openclaw/pull/108676) prepared incompatible mutations for the same durable record. Run [29891323049](https://github.com/openclaw/clawsweeper/actions/runs/29891323049) reproduced the same root cause for [`openclaw/openclaw#106482`](https://github.com/openclaw/openclaw/pull/106482). The separate run [29890705127](https://github.com/openclaw/clawsweeper/actions/runs/29890705127) failed on GitHub's `commit_refs` rejection and reported an inner recovery identity error. Unconditional cleanup succeeded in each failure path. |
| Duplicate-item claim hotfix                  | Complete; landed, deployed, and live-verified  | [`openclaw/clawsweeper#772`](https://github.com/openclaw/clawsweeper/pull/772) landed at `883ce9914e57733b18b96bcf70e9308a29c8e237`; CI [run 29891730857](https://github.com/openclaw/clawsweeper/actions/runs/29891730857), CodeQL [run 29891730842](https://github.com/openclaw/clawsweeper/actions/runs/29891730842), and dashboard deployment [run 29891983255](https://github.com/openclaw/clawsweeper/actions/runs/29891983255) passed. The public surface returned to `max_items=4` and a 60-second wait. Recovery run [29891989901](https://github.com/openclaw/clawsweeper/actions/runs/29891989901) selected four distinct items and published state commit [`7993abb2`](https://github.com/openclaw/clawsweeper-state/commit/7993abb2b64e71b81b9f30904fdb23182fb82558) with `materialized=4`, `accepted=4`, `retryable=0`, and `released=0`; contention remained 1,958 and open dead letters remained 428. |
| Size-8 re-advance                            | In review                                      | [`openclaw/clawsweeper#775`](https://github.com/openclaw/clawsweeper/pull/775) builds on the configured-cap safety hotfix [`a114b4e3`](https://github.com/openclaw/clawsweeper/commit/a114b4e3e173caefb0fb6c941ee48fd845fae5d5) and corrects [`openclaw/clawsweeper#778`](https://github.com/openclaw/clawsweeper/pull/778), which advanced the dashboard grant directly to 32 while leaving the workflow ask at 4. It restores the workflow publisher scan ask of 32 introduced by [`openclaw/clawsweeper#773`](https://github.com/openclaw/clawsweeper/pull/773), uses that ask only to widen the owner-homogeneous candidate scan, and reduces the dashboard's hard lease cap from 32 to the current size-8 checkpoint. The queue clamps actual ownership to the configured lease size, persists that effective cap with the durable lease so idempotent retries cannot change it across deploys or rollbacks, and returns it to the batch manifest and writer telemetry. During rolling deployment, the client accepts current main's `effective_max_items` response before falling back to the request used by older uncapped workers; during rollback, returned lease membership is also a safe lower bound so an existing eight-member lease cannot be described as size 4. Both fresh-schema and migrated-column definitions provide the same rollback-safe default, so the current-main insert remains writable after a rollback. It retains the 60-second wait plus logical-item admission serialization. This keeps a full eight-item lease at `configured_batch_size=8` instead of misreporting the scan ask as a 32-item writer batch; true size-32 mutation ownership remains deferred to [`openclaw/clawsweeper#770`](https://github.com/openclaw/clawsweeper/pull/770). Merge only after CI and CodeQL pass; after merge, require dashboard deployment and smoke to pass before beginning the keep-or-rollback observation window. The final keep gate requires one eight-distinct-item commit with eight accepted outcomes, zero retryable outcomes, no contention or dead-letter growth, and two consecutive five-minute windows with resolved at least arrivals plus falling pending depth and oldest age. |
