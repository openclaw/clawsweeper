...
export const bayLayoutScript = String.raw`
  ...
  function planBayArea(width,height,count){
    var gap=10,pad=5,cols=Math.max(1,Math.floor((width-2*pad+gap)/124));
    var cellWidth=(width-2*pad)/cols,cardWidth=Math.min(126,cellWidth-gap-6);
    var fullRows=Math.max(0,Math.floor((height-2*pad+gap)/120));
    var compact=count>cols*fullRows,cardHeight=compact?80:110;
    var capacity=cols*Math.max(1,Math.floor((height-2*pad+gap)/(cardHeight+gap)));
    var limit=Math.min(count,capacity),rows=Math.max(1,Math.ceil(limit/cols));
  ...
  function prepareBayLayout(){
    var grid=document.getElementById("stage-grid"),mobile=portraitLayout(),widths=bayLaneWidths(grid.clientWidth,STAGES.map(function(area){return areaRows(area).length;}));
    grid.style.gridTemplateColumns=mobile?"":widths.map(function(width){return width+"px";}).join(" ");
    STAGES.forEach(function(area,index){bayAreaPlans[area]=planBayArea(mobile?Math.min(380,grid.clientWidth):widths[index],grid.clientHeight-(mobile?124:150),areaRows(area).length);});
    ["completed","attention"].forEach(function(area){var width=mobile?Math.min(380,grid.clientWidth-16):document.getElementById("beach").clientWidth*.2-18;bayAreaPlans[area]=planBayArea(width,mobile?grid.clientHeight-124:area==="completed"?249:109,areaRows(area).length);});
    // Page-relative top avoids changing positions when a poll arrives after scrolling.
